      ...
      <b-modal id="new-appointment-modal" size="lg" title="New Video Appointment" @show="resetModal" @hidden="resetModal" @ok="handleSubmit">
        <span>
          <form ref="form">
            ...
            <b-form-datepicker id="datepicker" v-model="appointmentDetails.date" class="mb-2" />
            <label for="timepicker">Choose time</label>
            <b-form-timepicker id="timepicker" v-model="appointmentDetails.time" :now-button="true" locale="en" class="mb-2" />
            <b-form-checkbox v-show="smsSupported === 'true'" id="sendsmscheckbox" v-model="sendSMS" value="send" unchecked-value="dont_send">
              Send SMS with link
            </b-form-checkbox>
            <b-form-input v-show="sendSMS === 'send' && smsSupported === 'true'" id="number-input" v-model="phonenumber" placeholder="Phone number to send to" class="mt-2" />
          </form>
        </span>
      </b-modal>
    </div>
    <div class="appointments-table">
      <b-table striped hover responsive="sm" :fields="tableHeaders" :items="tableItems">
        <template v-slot:cell(guestURL)="row">
          <div>
            <span>Copy Guest URL</span>
            <b-button v-clipboard:success="onCopy" v-clipboard:copy="row.item.guestURL" size="sm" class="ml-2">
        ...
        <template v-slot:cell(hostURL)="row">
          <div>
            ...
            <b-button v-clipboard:success="onCopy" v-clipboard:copy="row.item.hostURL" size="sm" class="ml-2">
